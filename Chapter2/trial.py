...
def test(x,y,num,velo,vwind):

    an_1 = angl(x,y,velo,vwind,1)[0]
    err = []

    for i in range(num):
        
        velor = random.normalvariate(velo,0.05*velo)
        angler = random.normalvariate(an_1,0.01*an_1)
        vwindr = random.normalvariate(vwind,0.1*vwind)
        
        hit_x = get_x(y,velor,angler,vwindr)
        error = (hit_x - x)**2
        err.append(error)

    res = math.sqrt(float(sum(err)/num))
    return res
