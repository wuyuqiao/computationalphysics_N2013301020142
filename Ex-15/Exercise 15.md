...
班级：13级弘毅班  
## 摘要  
本文主要用Python语言对一系列随机过程做了模拟，通过观察数值计算的结果，表明数值计算方法在解决随机过程问题中的作用。文中涉及的随机过程首先为一维情况下的随机行走，分别讨论了左右概率相同和不同的情况。之后延伸到二维情况，并对二维系统中熵随时间变化的情况作了简单讨论  
### 关键词： 随机行走 平均值 熵  
## 一、背景介绍  
...
## 六、结论
通过Python语言对随机过程的模拟，我们可以看出其结果与概率论的结论是一致的。从而我们可以说，数值计算可以对随机过程有一个较好的模拟。
## 七、参考文献
